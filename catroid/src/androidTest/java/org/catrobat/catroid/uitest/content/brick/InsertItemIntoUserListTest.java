...
public class InsertItemIntoUserListTest extends BaseActivityInstrumentationTestCase<MainMenuActivity> {

    private Project project;
    private InsertItemIntoUserListBrick insertItemIntoUserListBrick;

    public InsertItemIntoUserListTest() {
    ...
    public void testCreateNewUserListAndDeletion() {
        ...
        assertNotNull("TextView does not exist.", solo.getText(solo.getString(R.string.brick_insert_item_into_userlist_insert_into)));
        ...
        assertTrue("ScriptFragment not visible", solo.waitForText(solo.getString(R.string.brick_insert_item_into_userlist_insert_into)));
        assertTrue("Created List not set in spinner", solo.searchText(userListName));

        UserList userList = (UserList) Reflection.getPrivateField(UserListBrick.class, insertItemIntoUserListBrick, "userList");
        assertNotNull("UserList is null", userList);

        solo.clickOnView(solo.getView(R.id.insert_item_into_userlist_spinner));
        ...
        userList = (UserList) Reflection.getPrivateField(UserListBrick.class, insertItemIntoUserListBrick, "userList");
        assertNotNull("UserList is null", userList);
        assertTrue("UserList Name not as expected", userList.getName().equals(secondUserListName));

        solo.clickOnView(solo.getView(R.id.brick_insert_item_into_userlist_value_edit_text));
        ...
        Spinner userListSpinner = (Spinner) UiTestUtils.getViewContainerByIds(solo, R.id.insert_item_into_userlist_spinner,
        ...
        assertTrue("UserList not set in spinner after deletion", solo.searchText(userListName));
        userListSpinner = (Spinner) solo.getView(R.id.insert_item_into_userlist_spinner);
        assertEquals("UserList count not as expected in spinner", 2, userListSpinner.getAdapter().getCount());
        userList = (UserList) Reflection.getPrivateField(UserListBrick.class, insertItemIntoUserListBrick, "userList");
        assertNotNull("UserList is null", userList);
    ...
    public void testCreateUserListInFormulaEditor() {
        ...
        solo.clickOnView(solo.getView(R.id.brick_insert_item_into_userlist_at_index_edit_text));
        solo.waitForFragmentByTag(FormulaEditorFragment.FORMULA_EDITOR_FRAGMENT_TAG);
        solo.clickOnView(solo.getView(R.id.formula_editor_keyboard_data));
        assertTrue("UserList Fragment not shown", solo.waitForText(solo.getString(R.string.formula_editor_data)));
    ...
    private void createProject() {
        ...
        insertItemIntoUserListBrick = new InsertItemIntoUserListBrick(10, 1);
        script.addBrick(insertItemIntoUserListBrick);
