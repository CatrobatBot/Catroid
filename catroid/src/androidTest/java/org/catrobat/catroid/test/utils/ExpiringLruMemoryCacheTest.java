...
public final class ExpiringLruMemoryCacheTest extends InstrumentationTestCase {
	...
	protected void setUp() throws Exception {
		...
		maxNumOfEntries = (2 * bitmap.getByteCount() / 1024); // allocate memory for exactly 2 bitmap files (in KB)
	...
	public void testFetchingNotExistingImageKeyShouldFail() {
		final String key = "a";
		assertNull("Fetching not-existing image entry must return null!", imageCache.get(key));
	}

	public void testFetchingImageExpiryTimeForNotExistingKeyShouldReturnZero() {
		final String key = "a";
		assertEquals("Expiry time of not-existing image must be 0!", 0, imageCache.getExpiryTime(key));
	}
	...
	public void testShouldReturnImageForNonExpiredImage() {
		...
		assertNotNull("Fetching non-expired image entry must NOT return null", resultA);
		assertEquals("Image height value of cached item changed unexpectedly", images[0].height, resultA.getHeight());
		assertEquals("Image width value of cached item changed unexpectedly", images[0].width, resultA.getWidth());
	}
	...
	public void testShouldReturnNullForExpiredImage() {
		...
		imageCache.put(key, value);
		testClock.elapsedTime += EXPIRE_TIME + 100; // simulate wait until key gets expired!
		assertNull("Cache unexpectedly returned an already expired image!", imageCache.get(key));
	}
	...
	public void testRemoveNonExpiredImage() {
		...
		imageCache.put(key, value);
		assertNotNull("Could not add image to cache", value);
		assertNotNull("remove() did not return removed image", imageCache.remove(key));
		assertNull("Removed image still available in cache", imageCache.get(key));
	}
	...
	public void testExpiryTimeForNonExpiredImage() {
		...
		assertTrue("Key does not exist or is not valid any more!", expiryTime != 0);
		assertEquals("Unexpected expiry time", testClock.elapsedTime + EXPIRE_TIME, expiryTime);
	}
	...
	public void testAccessingImageShouldNotIncreaseExpiryTime() {
		...
		testClock.elapsedTime += EXPIRE_TIME - 100; // simulate wait 200ms! key should remain valid!
		assertNotNull("Non expired image does not exists any more", imageCache.get(key));
		assertEquals("Key does not exist or expiry time changed unexpectedly!",
				initialExpiryTime, imageCache.getExpiryTime(key));

		testClock.elapsedTime += EXPIRE_TIME - 100; // wait another 200ms
		assertNull("Expired image must NOT be available any more", imageCache.get(key));
	...
	public void testRemovingExpiryTimeOfImage() {
		...
		imageCache.put(key, value);
		imageCache.removeExpiryTime(key);
		assertEquals("Expiry time of removed image must be 0!", 0, imageCache.getExpiryTime(key));
	}
	...
	public void testExceedingMaxSizeShouldEvictLeastRecentlyUsedImageEntryAndRemoveExpiryCacheImage() {
		...
		assertNotNull("Replaced entry still available, but should have been overwritten!", imageCache.get(keyC));
		assertTrue("Expiry time of replaced entry not updated!", imageCache.getExpiryTime(keyC) != 0);

		assertNotNull("Not-replaced entry has been removed unexpectedly!", imageCache.get(keyB));
		assertEquals("Expiry-time of not-replaced entry has been unexpectedly updated!",
				expiryTimeB, imageCache.getExpiryTime(keyB));

		assertNull("LRU algorithm did not removed least recently used entry", imageCache.get(keyA));
		assertEquals("LRU algorithm did not invalidate expiry time of least recently used entry",
				0, imageCache.getExpiryTime(keyA));
	}
}
