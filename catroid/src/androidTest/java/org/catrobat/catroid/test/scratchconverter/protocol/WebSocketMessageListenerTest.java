...
public class WebSocketMessageListenerTest extends AndroidTestCase {
	...
	public void testReceivingInfoMessage() {
		...
		expectedFirstJob.setDownloadState(Job.DownloadState.ALREADY_DOWNLOADED);
		...
		expectedSecondJob.setDownloadState(Job.DownloadState.NOT_YET_DOWNLOADED);
		...
		doAnswer(new Answer<Void>() {
			...
			public Void answer(InvocationOnMock invocation) throws Throwable {
				...
				assertEquals("Wrong downloadState value extracted of first job",
						expectedFirstJob.getDownloadState(), jobs[0].getDownloadState());
		...
		return new HashMap<String, Object>() {
			{
				...
				put(JsonKeys.JsonJobDataKeys.ALREADY_DOWNLOADED.toString(),
						job.getDownloadState() == Job.DownloadState.ALREADY_DOWNLOADED);
