...
public class GoToActionsTest extends AndroidTestCase {
	...
	public void testGoToOtherSpriteAction() throws InterruptedException {
		...
		Action action = sprite.getActionFactory().createGoToAction(sprite, destinationSprite,
				BrickValues.GO_TO_OTHER_SPRITE_POSITION);

		action.act(1f);

		assertEquals("Incorrect sprite x position after GoToAction executed", DESTINATION_X_POSITION,
				sprite.look.getXInUserInterfaceDimensionUnit());
		assertEquals("Incorrect sprite y position after GoToAction executed", DESTINATION_Y_POSITION,
				sprite.look.getYInUserInterfaceDimensionUnit());
	}

	public void testNullActor() {
		ActionFactory factory = new ActionFactory();
		Action action = factory.createGoToAction(null, null, 0);
		try {
			action.act(1.0f);
