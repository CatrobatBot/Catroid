...
public final class ExpiringLruDiskCacheTest extends InstrumentationTestCase {
	...
	private static final String TAG = ExpiringLruDiskCacheTest.class.getSimpleName();
	...
	public void testFetchingNotExistingImageKeyShouldFail() {
		...
		try {
			assertNull("Fetching a not existing image must return null", imageCache.getBitmap(key));
		} catch (IOException e) {
	...
	public void testShouldReturnImageForNonExpiredImage() {
		...
		try {
			...
			assertNotNull("Recently added entry not available any more!", entry);
			Bitmap bitmap = entry.getBitmap();
			assertNotNull("Image of recently added entry is corrupt!", bitmap);
			assertEquals("Image of recently added entry is corrupt! Height differs",
					bitmap.getHeight(), images[0].height);
			assertEquals("Image of recently added entry is corrupt! Weight differs",
					bitmap.getWidth(), images[0].width);
		} catch (IOException e) {
			fail("Cannot access cache!");
		} finally {
	...
	public void testRemoveNonExpiredImage() {
		...
		try {
			imageCache.put(key, inputStream);
			assertNotNull("Recently added entry not available any more!", imageCache.getBitmap(key));
			assertTrue("Unable to remove existing added entry!", imageCache.remove(key));
			assertNull("Removed entry still available!", imageCache.getBitmap(key));
		} catch (IOException e) {
			fail("Cannot access cache!");
		} finally {
			DiskLruCache.closeQuietly(inputStream);
	...
	public void testShouldReturnNullForExpiredImage() {
		...
		try {
			...
			assertNull("Expired entry still available!",
					imageCache.getBitmap(key));          // getBitmap() triggers 2nd clock access
		} catch (IOException e) {
			fail("Cannot access cache!");
		} finally {
			DiskLruCache.closeQuietly(inputStream);
	...
	public void testExpiryTimeForNonExpiredImage() {
		...
		try {
			...
			assertNotNull("Unexpired entry not available any more!", entry);
			...
			assertEquals("Wrong expiration date", startTime + EXPIRE_TIME, expirationDate);
		} catch (IOException e) {
			fail("Cannot access cache!");
		} finally {
			DiskLruCache.closeQuietly(inputStream);
	...
	public void testAccessingImageShouldNotIncreaseExpiryTime() {
		...
		try {
			...
			ExpiringDiskCache.BitmapEntry entry;
			entry = imageCache.getBitmap(key);              // getBitmap() triggers 2nd clock access
			assertNotNull("Unexpired entry got removed unexpectedly", entry);
			...
			assertNotNull("Unexpired entry is not available any more", entry);
			...
			assertNull("Key must not be valid any more!", entry);
		} catch (IOException e) {
			fail("Cannot access cache!");
		} finally {
			DiskLruCache.closeQuietly(inputStream);
	...
	public void testExceedingMaxSizeShouldEvictLeastRecentlyUsedImageEntryAndRemoveExpiryCacheImage() {
		...
		assertNotNull("Input stream is null!", inputStreamA);
		assertNotNull("Input stream is null!", inputStreamB);
		assertNotNull("Input stream is null!", inputStreamC);
		...
		try {
			...
			assertNotNull("Recently added entry is not available any more", entry);
			...
			assertNotNull("Image of cached image B got corrupt", imageCache.getBitmap(keyB));
			assertNotNull("Image of cached image B got corrupt", imageCache.getBitmap(keyB));
			assertNotNull("Image of cached image B got corrupt", imageCache.getBitmap(keyB));

			// now add another item, which should evict "a"
			...
			assertNotNull("Image of cached image B got corrupt", entry);
			...
			entry = imageCache.getBitmap(keyB);
			assertNotNull("LRU algorithm removed most-recently instead of least-recently used entry", entry);
			final long currentExpirationDateB = extractExpirationDateFromMetaData(entry.getMetadata());
			assertTrue("Expiration date does not exist or is not valid any more!", expirationDateB != 0);
			assertEquals("Expiry time changed unexpectedly!", currentExpirationDateB, expirationDateB);
		} catch (IOException e) {
			fail("Cannot access cache!");
		} finally {
	...
	// Stub classes
	private static final class EditorStub implements DiskCacheEntryEditorInterface {
	...
	private static final class SnapshotStub implements DiskCacheSnapshotInterface {
	...
	private static final class CacheBackendStub implements DiskCacheBackendInterface {
