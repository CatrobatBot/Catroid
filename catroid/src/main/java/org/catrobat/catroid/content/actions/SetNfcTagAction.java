...
public class SetNfcTagAction extends TemporalAction {
	...
	protected void update(float percent) {
		...
		try {
			message = NfcHandler.createMessage(nfcNdefMessage.interpretString(sprite), nfcTagNdefSpinnerSelection);
		} catch (InterpretationException e) {
			Log.d(getClass().getSimpleName(), "Couldn't create message", e);
		}
		try {
			StageActivity.addNfcTagMessageToDeque(message);
		} catch (Exception e) {
			Log.d(getClass().getSimpleName(), "Failed to add NfcTagMessage to Deque", e);
		}
	}
