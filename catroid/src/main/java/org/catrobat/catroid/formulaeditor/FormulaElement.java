...
public class FormulaElement implements Serializable {
	...
	public void updateCollisionFormulaToNewVersion() {
		...
		if (type == ElementType.COLLISION_FORMULA) {
			int indexOfSpriteInFormlua = -1;
			for (Scene scene : ProjectManager.getInstance().getCurrentProject().getSceneList()) {
				for (Sprite sprite : scene.getSpriteList()) {
					int index = value.indexOf(sprite.getName());
					if (index > indexOfSpriteInFormlua) {
						indexOfSpriteInFormlua = index;
					}
				}
			}
			if (indexOfSpriteInFormlua < 0) {
				return;
			}
			String secondSpriteName = value.substring(indexOfSpriteInFormlua, value.length());
