...
public abstract class BackPackActivityFragment extends CheckBoxListFragment implements CheckBoxListAdapter.ListItemLongClickHandler {
	...
	protected void startActionMode(ActionMode.Callback actionModeCallback) {
		...
		if (adapter.isEmpty()) {
			if (actionModeCallback.equals(unpackModeCallBack)) {
				showEmptyActionModeDialog(R.string.unpack, R.string.nothing_to_unpack);
