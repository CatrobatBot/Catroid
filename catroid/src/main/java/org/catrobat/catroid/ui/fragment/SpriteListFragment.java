...
public class SpriteListFragment extends ListActivityFragment implements CheckBoxListAdapter.ListItemClickHandler<Sprite> {
	...
	public void showReplaceItemsInBackPackDialog() {
		if (BackPackSpriteController.existsInBackPack(spriteAdapter.getCheckedItems())) {
			showReplaceItemsInBackPackDialog(spriteAdapter.getCheckedItems().get(0).getName());
		} else {
			packCheckedItems();
		}
	}
