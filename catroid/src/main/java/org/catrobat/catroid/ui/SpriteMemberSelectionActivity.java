...
public class SpriteMemberSelectionActivity extends Activity implements RenameItemDialog.RenameItemInterface {
	...
	private void setActionBarTitle(String spriteName) {
		boolean multipleScenes = ProjectManager.getInstance().getCurrentProject().getSceneList().size() > 1;
	...
	public void handleNfcTagsButton(View view) {
		Intent intent = new Intent(this, ScriptActivity.class);
		intent.putExtra(ScriptActivity.EXTRA_FRAGMENT_POSITION, ScriptActivity.FRAGMENT_NFCTAGS);
		startActivity(intent);
	}
