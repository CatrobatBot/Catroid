 ...
 * We use this as a safety to prevent bad midiDriver files from playing forever.
 *
 * Inputs:
 * psEASData - pointer to overall EAS data structure
 *
 ...
 * nKeyNumber - new note's midiDriver note number
 ...
 *
 * Note: some midiDriver disassemblers display a bipolar pitch bend value.
...
EAS_RESULT VMRender (S_VOICE_MGR *pVoiceMgr, EAS_I32 numSamples, EAS_I32 *pMixBuffer, EAS_I32 *pVoicesRendered)
{
    ...
    *pVoicesRendered = VMAddSamples(pVoiceMgr, pMixBuffer, numSamples);
     ...
     * midiDriver note-off messages because the midiDriver note-on and corresponding midiDriver
